...
# Kerberos principal
SETPRINCIPAL="dhcpdupdater@${REALM}"
# Kerberos keytab
# /etc/dhcpdupdater.keytab
# krbcc ticket cache
# /tmp/dhcp-dyndns.cc
TESTUSER="$($WBINFO -u) | grep 'dhcpdupdater')"
if [ -z "${TESTUSER}" ]; then
    ...
    logger "samba-tool user create dhcpdupdater --random-password --description=\"Unprivileged user for DNS updates via ISC DHCP server\""
    logger "samba-tool user setexpiry dhcpdupdater --noexpiry"
    logger "samba-tool group addmembers DnsAdmins dhcpdupdater"
    exit 1
fi

# Check for Kerberos keytab
if [ ! -f /etc/dhcpdupdater.keytab ]; then
    echo "Required keytab /etc/dhcpdupdater.keytab not found, it needs to be created."
    echo "Use the following commands as root"
    echo "samba-tool domain exportkeytab --principal=${SETPRINCIPAL} /etc/dhcpdupdater.keytab"
    echo "chown XXXX:XXXX /etc/dhcpdupdater.keytab"
    echo "Replace 'XXXX:XXXX' with the user & group that dhcpd runs as on your distro"
    echo "chmod 400 /etc/dhcpdupdater.keytab"
    exit 1
fi
...
if [ "$?" != "0" ]; then
    ...
    kinit -F -k -t /etc/dhcpdupdater.keytab -c /tmp/dhcp-dyndns.cc "${SETPRINCIPAL}"
    if [ "$?" != "0" ]; then
